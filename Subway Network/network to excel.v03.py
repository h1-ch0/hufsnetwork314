...
def inputSequence():
    ...
    out body;
    """
    return area_search, date, routeType, query
#todo try to define a function to input sequence of stations
#! 클래스 하면
class SubwayNetwork:
    def __init__(self, area_search, date, routeType, query):
    ...
    def run(self):
        subway_data = extract_osm_geodata(self.query)
        ...
        m.save(f"{self.area_search}_{self.routeType}_map_{self.date}.html")
        export_nodes_to_excel(G, filename=f"{self.area_search}_subway_nodes_{self.date}.xlsx")
        export_edges_to_excel(G, filename=f"{self.area_search}_subway_edges_{self.date}.xlsx")
        visualize_graph(G, f"Test_{self.area_search} Subway Network")
